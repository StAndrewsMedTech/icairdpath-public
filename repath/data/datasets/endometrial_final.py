...
class Endometrial(Dataset):
    ...
    def labels(self) -> Dict[str, int]:
        return {"background" : 0, "other_benign": 1 , "malignant": 2, "blood": 3, "mucus": 4, "bloodmucus": 5 }
...
def train():
    ...
    csv_path = metadata_dir / "iCAIRD_Endometrial_7.csv"

    endometrial_df = pd.read_csv(csv_path)
    endometrial_df = endometrial_df[endometrial_df['use_row'] == 1]
    endometrial_df = endometrial_df[endometrial_df['train_valid_test'] == 'train']
    endometrial_df = endometrial_df.reset_index(drop=True)

    def annotation_rel_path(df_row):
        batch_no = df_row.batch
        image_name = str(df_row.Filename)
        annot_name = image_name[:-7] + "txt"
        annot_name = '/Endometrial/' + annot_name
        return annot_name
    ...
    df["tags"] = endometrial_df['subCategory'] + ";" + endometrial_df['train_valid_test']
...
def valid():
    ...
    endometrial_df = pd.read_csv(csv_path)
    endometrial_df = endometrial_df[endometrial_df['use_row'] == 1]
    endometrial_df = endometrial_df[endometrial_df['train_valid_test'] == 'valid']
    endometrial_df = endometrial_df.reset_index(drop=True)
...
def test():
    ...
    endometrial_df = pd.read_csv(csv_path)
    endometrial_df = endometrial_df[endometrial_df['use_row'] == 1]
    endometrial_df = endometrial_df[endometrial_df['train_valid_test'] == 'test']
    endometrial_df = endometrial_df.reset_index(drop=True)
